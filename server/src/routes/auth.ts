...
authRouter.post('/register', (req: any, res: any) => {
  ...
  if (existing) return res.status(409).json({ error: 'Email o usuario ya existe' })

  const id = randomUUID()
  const password_hash = bcrypt.hashSync(password, 10)
  db.prepare('INSERT INTO users (id, email, password_hash, username) VALUES (?, ?, ?, ?)')
    .run(id, email, password_hash, username)
  db.prepare(`INSERT INTO profiles (user_id, nombre_completo, is_public, seisena, patrulla, equipo_pioneros, comunidad_rovers, fecha_nacimiento, rol_adulto, telefono, avatar_url) 
...
authRouter.post('/login', (req: any, res: any) => {
  ...
  const row = db.prepare('SELECT id, password_hash, username FROM users WHERE email = ?').get(email)
