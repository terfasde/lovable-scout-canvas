...
adminRouter.post('/users/delete', adminGate, (req: any, res: any) => {
  ...
  if (!isSecretValid(req)) {
    ...
    if (!current || !isAdminEmail(current.email)) return res.status(403).json({ error: 'No autorizado' })
  }

  const target = db.prepare('SELECT id FROM users WHERE email = ?').get(emailToDelete) as { id?: string } | undefined
  if (!target?.id) return res.status(404).json({ error: 'Usuario no encontrado' })
