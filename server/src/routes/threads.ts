...
threadsRouter.post('/', authMiddleware, (req: any, res: any) => {
  ...
  const id = randomUUID()
  db.prepare(`INSERT INTO threads (id, author_id, content, image_url) VALUES (?, ?, ?, ?)`)
    .run(id, userId, trimmed, image_url ?? null)
  const row = db.prepare(`SELECT id, author_id, content, image_url, created_at FROM threads WHERE id = ?`).get(id)
...
threadsRouter.delete('/:id', authMiddleware, (req: any, res: any) => {
  ...
  const thread = db.prepare(`SELECT author_id FROM threads WHERE id = ?`).get(id)
  ...
  if (thread.author_id !== userId) {
    ...
    if (!u || !isAdminEmail(u.email)) return res.status(403).json({ error: 'No autorizado' })
  }
...
threadsRouter.post('/:id/comments', authMiddleware, (req: any, res: any) => {
  ...
  if (!parse.success) return res.status(400).json({ error: parse.error.flatten() })
  const id = randomUUID()
  db.prepare(`INSERT INTO thread_comments (id, thread_id, author_id, content) VALUES (?, ?, ?, ?)`)
    .run(id, threadId, userId, parse.data.content.trim())
  const row = db.prepare(`SELECT id, thread_id, author_id, content, created_at FROM thread_comments WHERE id = ?`).get(id)
  res.status(201).json(row)
})

// Eliminar comentario (autor del comentario o autor del hilo)
threadsRouter.delete('/:id/comments/:commentId', authMiddleware, (req: any, res: any) => {
  const threadId = String(req.params.id)
  const commentId = String(req.params.commentId)
  const userId = (req as any).user.id as string
  const comment = db.prepare(`SELECT author_id, thread_id FROM thread_comments WHERE id = ?`).get(commentId)
  if (!comment || comment.thread_id !== threadId) return res.status(404).json({ error: 'Comentario no encontrado' })
  if (comment.author_id !== userId) {
    const thread = db.prepare(`SELECT author_id FROM threads WHERE id = ?`).get(threadId)
    if (!thread || thread.author_id !== userId) return res.status(403).json({ error: 'No autorizado' })
  }
