...
groupsRouter.post('/', authMiddleware, (req: any, res: any) => {
  ...
  db.prepare('INSERT INTO groups (id, name, description, cover_url, creator_id) VALUES (?, ?, ?, ?, ?)')
...
groupsRouter.post('/:id/leave', authMiddleware, (req: any, res: any) => {
  ...
  if (member.role === 'owner') return res.status(400).json({ error: 'El propietario no puede salir. Transfiere propiedad o elimina el grupo.' })
  db.prepare('DELETE FROM group_members WHERE group_id = ? AND user_id = ?').run(groupId, userId)
  res.json({ ok: true })
})

const inviteSchema = z.object({ userIds: z.array(z.string().uuid()).min(1) })
groupsRouter.post('/:id/members/invite', authMiddleware, (req: any, res: any) => {
  const groupId = req.params.id
  const userId = (req as any).user.id as string
  const me = db.prepare('SELECT role FROM group_members WHERE group_id = ? AND user_id = ?').get(groupId, userId)
  if (!me || (me.role !== 'owner' && me.role !== 'admin')) return res.status(403).json({ error: 'Permisos insuficientes' })
...
groupsRouter.delete('/:id', authMiddleware, (req: any, res: any) => {
  const groupId = req.params.id
  const userId = (req as any).user.id as string
  const owner = db.prepare('SELECT 1 FROM group_members WHERE group_id = ? AND user_id = ? AND role = ?').get(groupId, userId, 'owner')
  if (!owner) return res.status(403).json({ error: 'Solo el propietario puede eliminar' })
  db.prepare('DELETE FROM groups WHERE id = ?').run(groupId)
  res.json({ ok: true })
})

const messageSchema = z.object({ content: z.string().max(4000).optional(), image_url: z.string().url().optional() })
groupsRouter.post('/:id/messages', authMiddleware, (req: any, res: any) => {
  ...
  if (!content && !image_url) return res.status(400).json({ error: 'Mensaje vacío' })
  const id = randomUUID()
  db.prepare('INSERT INTO group_messages (id, group_id, user_id, content, image_url) VALUES (?, ?, ?, ?, ?)')
...
groupsRouter.get('/:id/messages', authMiddleware, (req: any, res: any) => {
  ...
  let rows
  if (since) {
    rows = db.prepare('SELECT * FROM group_messages WHERE group_id = ? AND datetime(created_at) > datetime(?) ORDER BY created_at ASC').all(groupId, since)
  } else {
    rows = db.prepare('SELECT * FROM group_messages WHERE group_id = ? ORDER BY created_at ASC').all(groupId)
  }
  res.json(rows)
})
...
groupsRouter.post('/:id/admins/promote', authMiddleware, (req: any, res: any) => {
  const groupId = req.params.id
  const me = (req as any).user.id as string
  const meRow = db.prepare('SELECT role FROM group_members WHERE group_id = ? AND user_id = ?').get(groupId, me) as { role?: string } | undefined
  if (!meRow || meRow.role !== 'owner') return res.status(403).json({ error: 'Solo el propietario puede promover' })
  ...
  if (target.role === 'owner') return res.status(400).json({ error: 'No puedes cambiar el propietario' })
  db.prepare('UPDATE group_members SET role = ? WHERE group_id = ? AND user_id = ?').run('admin', groupId, userId)
  res.json({ ok: true })
})

groupsRouter.post('/:id/admins/demote', authMiddleware, (req: any, res: any) => {
  const groupId = req.params.id
  const me = (req as any).user.id as string
  const meRow = db.prepare('SELECT role FROM group_members WHERE group_id = ? AND user_id = ?').get(groupId, me) as { role?: string } | undefined
  if (!meRow || meRow.role !== 'owner') return res.status(403).json({ error: 'Solo el propietario puede degradar' })
  const parse = roleBodySchema.safeParse(req.body)
  if (!parse.success) return res.status(400).json({ error: parse.error.flatten() })
  const { userId } = parse.data
  const target = db.prepare('SELECT role FROM group_members WHERE group_id = ? AND user_id = ?').get(groupId, userId) as { role?: string } | undefined
...
groupsRouter.delete('/:id/members/:userId', authMiddleware, (req: any, res: any) => {
  const groupId = req.params.id
  const userId = req.params.userId
  const me = (req as any).user.id as string
  const meRow = db.prepare('SELECT role FROM group_members WHERE group_id = ? AND user_id = ?').get(groupId, me) as { role?: string } | undefined
  if (!meRow || (meRow.role !== 'owner' && meRow.role !== 'admin')) return res.status(403).json({ error: 'Permisos insuficientes' })
  const target = db.prepare('SELECT role FROM group_members WHERE group_id = ? AND user_id = ?').get(groupId, userId) as { role?: string } | undefined
  if (!target) return res.status(404).json({ error: 'Miembro no encontrado' })
  if (target.role === 'owner') return res.status(400).json({ error: 'No puedes expulsar al propietario' })
  db.prepare('DELETE FROM group_members WHERE group_id = ? AND user_id = ?').run(groupId, userId)
  res.json({ ok: true })
})
...
groupsRouter.put('/:id', authMiddleware, (req: any, res: any) => {
  ...
  const params: any[] = []
  if (name !== undefined) { setParts.push('name = ?'); params.push(name) }
  if (description !== undefined) { setParts.push('description = ?'); params.push(description) }
  if (cover_url !== undefined) { setParts.push('cover_url = ?'); params.push(cover_url) }
