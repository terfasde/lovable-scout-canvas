...
export async function deleteGroupDeep(groupId: string): Promise<void> {
  ...
  const [{ data: group, error: gErr }, { data: msgs, error: mErr }] = await Promise.all([
    supabase.from('groups').select('cover_image').eq('id', groupId).maybeSingle(),
    supabase.from('group_messages').select('image_url').eq('group_id', groupId).not('image_url', 'is', null),
  ]);
  if (gErr) throw gErr;
  if (mErr) throw mErr;
...
export async function inviteMembers(groupId: string, userIds: string[]): Promise<{ inserted: number; skipped: number; }>{
  ...
  const rows = toInsert.map(uid => ({ group_id: groupId, user_id: uid, role: 'member' as const }));
