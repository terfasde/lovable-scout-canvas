...
export async function listImages(album: string): Promise<string[]> {
  const { data, error } = await supabase.storage.from(BUCKET).list(album, { limit: 1000 });
  if (error) throw error;
  const files = (data || []).filter((f: any) => !f.name.endsWith("/"));
  const urls = files
    .map((f: any) => supabase.storage.from(BUCKET).getPublicUrl(`${album}/${f.name}`).data.publicUrl)
