...
# 
# ⚠️ ACTUALIZADO: Proyecto sin Supabase, backend opcional
#
# Uso: .\start.ps1 [modo]
#   .\start.ps1         - Modo frontend solo (RECOMENDADO)
#   .\start.ps1 dev     - ⚠️ Legacy: Requiere backend configurado
#   .\start.ps1 full    - ⚠️ Legacy: Arquitectura completa (requiere configuración)
...
Write-Host "Docker Compose - Aplicacion Scout (Sin Supabase)" -ForegroundColor Cyan
Write-Host "===================================================" -ForegroundColor Cyan
Write-Host ""

function Show-Menu {
    Write-Host "Modos disponibles:" -ForegroundColor Yellow
    Write-Host "  simple - Solo frontend con mocks locales (RECOMENDADO)" -ForegroundColor Green
    Write-Host "  dev    - ⚠️ Legacy: Desarrollo con backend (requiere configuración)" -ForegroundColor DarkYellow
    Write-Host "  full   - ⚠️ Legacy: Arquitectura completa (requiere configuración)" -ForegroundColor DarkYellow
...
function Start-FullMode {
    Write-Host "⚠️ MODO LEGACY - Requiere configuración completa" -ForegroundColor DarkYellow
    Write-Host ""
    Write-Host "Este modo requiere:" -ForegroundColor Yellow
    Write-Host "  1. PostgreSQL configurado" -ForegroundColor White
    Write-Host "  2. Backend con dependencias instaladas" -ForegroundColor White
    Write-Host "  3. Prometheus y Grafana configurados" -ForegroundColor White
    Write-Host "  4. Variables de entorno actualizadas" -ForegroundColor White
    Write-Host ""
    Write-Host "⚠️ Este archivo está DESACTUALIZADO para el proyecto actual." -ForegroundColor Red
    Write-Host ""
    $confirm = Read-Host "¿Continuar de todos modos? (s/N)"
    if ($confirm -ne 's') {
        Write-Host "Cancelado. Usa '.\start.ps1 simple' en su lugar." -ForegroundColor Cyan
